...
- Wi-Fi upgrade/complimentary entitlement endpoints, event and consent endpoints, and an OpenAPI description.
...
Set `PACKETFENCE_API_URL` and `PACKETFENCE_API_TOKEN` to use the REST gateway. If the URL is unset, the service uses an in-memory gateway for development only. The current executable wires the mock PMS and mock folio intentionally; production PMS and payment connectors must be configured and selected before deployment.
